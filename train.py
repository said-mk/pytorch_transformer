...
from dataset import HausaVGDataset, causal_mask, get_or_build_tokenizer
...
def run_validation(model, val_dataloader, tokenizer_src, tokenizer_tgt, max_len, device, print_msg, global_step, writer, num_examples=2):
    ...
    if writer:
        ...
        cer_score = cer(predicted, expected)
        writer.add_scalar('validation/cer', cer_score, global_step)
        
        bleu = torchmetrics.text.BLEUScore()
        bleu_score = bleu(predicted, [[e] for e in expected])
...
def get_ds(config):
    ...
    ds_raw = load_dataset("parquet", data_files=f"hf://datasets/{config['datasource']}@refs/convert/parquet/default/train/*.parquet", split='train')
    ...
    train_ds = HausaVGDataset(train_ds_raw, tokenizer_src, tokenizer_tgt, config['lang_src'], config['lang_tgt'], config['seq_len'])
    val_ds = HausaVGDataset(val_ds_raw, tokenizer_src, tokenizer_tgt, config['lang_src'], config['lang_tgt'], config['seq_len'])
